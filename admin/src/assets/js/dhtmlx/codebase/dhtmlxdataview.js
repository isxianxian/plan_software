...
dhtmlx.MouseEvents={
	_init: function(){
		...
		if (this.on_mouse_move){
			...
			dhtmlx.event(this._obj,(_isIE?"mouseleave":"mouseout"),this._onMouse,this);
		}
...
dhtmlx.RenderStack={
	...
	_getDimension:function(){
    	  var m=0,n=0, a = this.type, b,d;
    	  if(typeof(a.margin) == "string" ){
    	  	var s = a.margin.replace(/px/g, '')._dhx_trim().split(" ");
    	  	if(s.length==4){
    	  	  m = s[1]*1 + s[3]*1
    	  	  n = s[0]*1 + s[2]*1
    	    }
    	  ...
    	  b = (a.border || 0) + (a.padding || 0) * 2 + m;
    	  d = (a.border || 0) + (a.padding || 0) * 2 + n;
        return {x: a.width + b,y: a.height + d}
	},
...
dhtmlx.DataDriver.xml={
	...
	fromString:function(xmlString){
		if (window.DOMParser && !_isIE)		// FF, KHTML, Opera
